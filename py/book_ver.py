...
def change_book_line_if_marked(line, now_version, now_short):
    ...
    if 'book-new-commit-version-squished-utc-date-time-place-marker' in line:
        # new = '    %i \\textperiodcentered \\textperiodcentered \\textperiodcentered ~%s \\textperiodcentered \\textperiodcentered \\textperiodcentered ~%s \\\\ [2.0em] %% book-new-commit-version-squished-utc-date-time-place-marker\n' % (counter+1, now_version, branch)
...
def replace_entries(now_version, now_short, orig_path, ignore_path, func_change_line_if):
    ...
    with open(ignore_path, 'r') as old:
        with open(orig_path, 'w') as orig:
            for line in old:
                # new = func_change_line_if(line, id_, date_time_, now_version, now_short, counter, branch)
...
def replace_all_entries(root_path):
