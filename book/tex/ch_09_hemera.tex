...
\end{wrapfigure}
Scout's horizontal and vertical steps can be blocked by any non-transparent piece,
i.e. any but Wave. When step is blocked, Scout can choose one of associated diagonal,
...
previous choices.
Dark Wave is transparent, non-blocking piece; so, light Scout does not get to
choose any forking steps, and can only pass-through ("step over") dark Wave.
...
\hyperref[fig:scn_mv_24_wave_activated_by_capture_pawn]{Wave activated by Pawn on its capture-step},
...
\indent
Here, activated Wave (now "in-the-air") can activate light Pyramid B with 5 momentum;
...
to the right; these are all only movement steps. Wave can also choose Grenadier’s
...
% \hyperref[fig:scn_n_21_sideways_pawns_cascade_pyramids]{the same as in cascade with a Pawn}.
