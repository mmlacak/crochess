...
(green, blue) as Unicorn has. For comparison, Knight's step-fields are also numbered (grey).
...
of them, regardless of a color.

On initial step, Centaur can freely choose any step, regardless of a color. On second step,
Centaur can choose any step-field in the other color (blue if green was chosen initially, green
if blue was first choice). On all subsequent steps, Centaur has to keep alternating between the
two initially chosen steps, for the remainder of a ply.
...
\begin{figure}[!h]
% \begin{figure}[!t]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/14_hd/scn_hd_08_scout_pawns.png}
\caption{Scout Pawns}
