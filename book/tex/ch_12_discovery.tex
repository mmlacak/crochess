...
all step-fields in its ply must be empty.
% Monolith moves similar to Knight,
...
step-fields form slightly larger pattern than in preceding step. New, larger pattern
contains fields neighbouring step-fields of preceding step. Step-fields of all
patterns (steps) are in a color opposite to Monolith's current field. So, each
succeeding diamond-shaped pattern has sides longer by two fields, compared to its
preceding step.
% For comparison, step-fields of a previous pattern are enumerated.
...
Any step-field can be chosen, regardless of any previous choice.
...
Monolith cannot be captured, converted, activated, or displaced by Serpent,
or Shaman. Pieces can only teleport from a Monolith; except Kings, Stars,
and Monoliths which cannot teleport.
...
\noindent
\begin{figure}[!h]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/20_d/scn_d_09_monolith_off_board.png}
\vspace*{-1.3\baselineskip}
\caption{Monolith off-board}
\label{fig:scn_d_09_monolith_off_board}
\end{figure}
