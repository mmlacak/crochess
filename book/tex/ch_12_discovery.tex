...
step-fields of a single step form a diamond-shaped pattern, or rhombus. Step-fields
of all patterns (steps) are always in a color opposite to the color of Monolith's
current field. Each consecutive step has slightly larger pattern, pattern growth is
not limited. Monolith can take steps beyond the four shown here.
...
to \hyperref[fig:scn_aoa_02_unicorn_opposite_color]{Unicorn's long jump}. For comparison,
step-fields of a first Monolith's step are enumerated. \newline
\indent
...
diamond-shaped pattern (rhombus) has its sides longer by two.
