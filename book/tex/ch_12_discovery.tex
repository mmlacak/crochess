...
One own figure can be (but don't have to be) demoted to Pawn, immediately after Monolith has stepped into syzygy.
Demoting to Pawn can be done even if no own Pawn has been captured yet. Opponent pieces, Kings, Stars, Monoliths
cannot be demoted.
...
To be granted option to demote own figure, Monolith must move into syzygy from an ordinary, non-syzygy field. Monolith
which is already in a syzygy can move into the same alignment, but cannot demote any figure. To get option to demote
again, Monolith has to move out of alignment first, and then on a next move Monolith can reenter syzygy.
...
The same applies even if Monolith moves into an alignment from completely independent syzygy, i.e. if the two does not
share neither any syzygy-fields nor celestial pieces. Again, to be able to demote own figure again, Monolith has to move
to an ordinary, non-syzygy field in a first move, then on a second move Monolith can enter the other syzygy.
