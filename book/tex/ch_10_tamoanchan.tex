...
\begin{figure}[!h]
% \begin{figure}[!t]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/16_tr/scn_tr_21_pawn_sacrifice_init.png}
\caption{Pawn-sacrifice start}
\label{fig:scn_tr_21_pawn_sacrifice_init}
% \centering
\end{figure}

Pawn-sacrifice move is initiated by Serpent activating Pyramid, which then
captures field at which own Pawn is located. Pawn is then oblationed (removed
from chessboard as if captured by opponent), and Serpent gets Pawn-sacrifice
tag and can, in the same move, start a new ply, as if starting a new move
altogether. Any of pieces involved can be on any side of chessboard, own or
opponent's.
...
\begin{figure}[!h]
% \begin{figure}[!t]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/16_tr/scn_tr_22_pawn_sacrifice_end.png}
\caption{Pawn-sacrifice end}
\label{fig:scn_tr_22_pawn_sacrifice_end}
% \centering
\end{figure}
...
traversing empty fields, e.g. teleporting, activating Wave, ...
