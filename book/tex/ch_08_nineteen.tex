...
\vspace*{-0.5\baselineskip}
Wave, activated by a Pawn moving sideways, can move horizontally (to the left, or
right), or vertically, towards opponent's figure row. Unlike Pawn, Wave is not
limited to only one step, and so can move to the end of chessboard.

Wave activated by a piece has the same choices of direction as activating piece.
So, Wave activated by a Pawn on its step-fields moves the same, regardles if Pawn
stepped forward, or sideways (i.e. horizontally, or vertically towards opponent).
