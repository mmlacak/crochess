...
\hyperref[fig:scn_mv_34_activating_rush_pawn_init]{rush light Pawn for 2 fields}.
...
\hyperref[fig:scn_mv_25_wave_activation_by_unicorn_first_step]{Wave activated by Unicorn}
...
by a Pawn on its step-field moves the same as stepping Pawn, regardless if that
...
\noindent
\begin{figure}[!h]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/12_n/scn_n_18_sideways_pawn_does_not_activate_pyramid.png}
\vspace*{-1.3\baselineskip}
...
Pawn
\hyperref[fig:scn_mv_36_activating_pyramid_by_pawn]{cannot activate Pyramid on its step-fields},
...
\noindent
\begin{figure}[!h]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/12_n/scn_n_19_sideways_pawns_cascade_pyramids.png}
\caption{Pyramids cascaded by sideways Pawns}
\label{fig:scn_n_19_sideways_pawns_cascade_pyramids}
\end{figure}

Similarly
\hyperref[fig:scn_mv_37_activating_pyramid_cascade_pawn]{to previous example},
...
can only castle on their first move, King cannot end its movement on an attacked field,
