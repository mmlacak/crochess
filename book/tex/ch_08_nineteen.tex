...
immediately to its left, or to its right. Sideways fields are step-fields; destination
...
Wave, activated by a Pawn moving sideways, can move only horizontally, either to
the left, or to the right. Unlike Pawn, Wave is not limited to only one step, and
so can move until edge of chessboard is encountered.
...
Image above and the next one both have two examples presented in parallel, on the top,
and to the bottom.

Pawns (and indeed, all the other pieces)
\hyperref[fig:scn_mv_30_activating_pyramid_by_pawn]{cannot activate Pyramid on their step-fields}.
Since sideways fields are also step-fields, Pyramid can't be activated by a sideways
moving Pawn, neither directly (top) nor indirectly (bottom), regardless how many Waves
where used for indirection.
...
\noindent
\begin{figure}[!h]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/12_n/scn_n_18_sideways_pawns_do_activate_pyramids.png}
\caption{Pyramids activated by sideways Pawns}
\label{fig:scn_n_18_sideways_pawns_do_activate_pyramids}
\end{figure}

. . . Pyramid can be activated if last active piece is not sideways Pawn, even if cascade contains sideways Pawn

\textgreater\textgreater\textgreater{} Activating Pyramid by cascading Pawn
...
\noindent
\begin{figure}[!h]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/12_n/scn_n_99_pawn_ranks.png}
\caption{Pawn rows}
\label{fig:scn_n_99_pawn_ranks}
\end{figure}
