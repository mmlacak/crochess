...
Losing ability to rush is denoted with \alg{::} (double colon) after piece, but before (destination)
field. Usually, this is used when Pawn is captured, converted, activated, etc., not when it was moved.
For instance, in this \hyperref[fig:scn_n_03_teleport_move_2]{teleporting example}, after dark Rook's
failed teleportation, light Bishop could capture dark Pawn on its initial position, which would be
denoted as \alg{Br17*P::}, and in \algfmt{XAN} it would be \alg{Bq18-r17*P::}.
...
Losing ability to castle is denoted with \alg{\&\&} (double ampersand) after piece, but before
(destination) field. Using previous examples, if Rooks moved to their destination fields without
castling, it would be written as \alg{R\&\&e1}, and \alg{R\&\&q1}, and in \algfmt{XAN} it would
be \alg{Ra1\&\&e1}, and \alg{Ry1\&\&q1}. If Rook in the first example was captured by opponent's
Bishop, it would be written as \alg{Ba1*R\&\&}.
...
sign) after piece, but before (destination) field, like so \alg{==e12}. In \algfmt{XAN}
the same move would be noted as \alg{Pe11==e12}.
...
5           & \alg{\&\&}           & lost castling tag                      \\ \cmidrule{1-3}
6           & \alg{:}              & en passant                             \\
7           & \alg{\&}             & castling                               \\
8           & \alg{=}              & promotion, tag                         \\
9           & \alg{\%}             & conversion                             \\
10          & \alg{\%\%}           & failed conversion, oblation            \\
11          & \alg{>}              & syzygy, demoting to Pawn               \\
12          & \alg{\$}             & syzygy, resurrection                   \\
13          & \alg{\$\$}           & failed resurrection                    \\
\bottomrule
\end{tabular}
...
these are called ply side-effects. Capturing, displacement and losing tags (promotion, rushing, or castling)
are both step and ply side-effects, others are purely ply side-effects.
...
\alg{Re2\~{}Wc2\~{}P::b3*N}, more verbose notation would be \alg{[Re6..e2]\~{}[We2..c2]\~{}[P::c2.b3*N]},
and in \algfmt{XAN} it would be \alg{[Re6-e2]\~{}[We2-c2]\~{}[P::c2-b3*N]}.
...
This means, at least some side-effects are now mandatory. To
...
(underscore). Choices are also valid regardless of formatting, spacing, e.g.
\begin{alltt}
<abc> = \alg{def}
    \alg{ghi}
  | \alg{jkl}
...
\end{alltt}
has two choices. Choice inside option group has 2 possible values, either \alg{f}
or \alg{g}. Global choice (not contained in any group) has 3 possible values: \alg{d},
\alg{e}\algfmt{[}\alg{f}\algfmt{|}\alg{g}\algfmt{]}\alg{h} or \alg{i}. This gives
\alg{d}, \alg{efh}, \alg{egh}, \alg{eh} and \alg{i} as possible values for
\algfmt{<abc>}.
...
<trance-journey> =
  <shaman-trance-journey-cascade>
    ...
    \{<step>[\alg{<}[<disposable-piece>]<field>]\}
| <shaman-trance-journey-cascade>
    \alg{@H}<trance-journey-start>
...
<pawn-sacrifice-captures> =
  \alg{S}[<disambiguation>]\{\alg{.}[\alg{.}]<field>[\alg{*}[\alg{P}[\alg{==}]]]\}
    [<pawn-sacrifice-end>]
