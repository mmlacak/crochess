...
\indent
For instance, in
\hyperref[fig:scn_o_19_starchild_divergence_end]{this example} light Queen
...
\footnotetext[3]{Two alternating directions.}
...
Activated Wave generally has the same set of initial step(s) as activating piece,
and can take any initial step, even if activating piece would be blocked, or if
opponent's piece is not present. For instance, Wave activated by Pawn can take
diagonal step even if destination field is empty.
...
Wave activated by Pawn moves toward opponent's figure row, either in a straight
line, or diagonally; once direction is chosen, it can't be changed for the
remainder of Pawn's ply. In later variants (Nineteen, and onwards)
\hyperref[fig:scn_n_16_sideways_pawn_init]{Pawn can move laterally by one step-field};
activated Wave on a step-field can now also move sideways.
