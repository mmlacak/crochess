...
\hyperref[fig:scn_o_20_starchild_conversion_immunity_init]{conversion immunity} would be
...
(tilde), before \hyperref[fig:scn_o_21_light_starchild_init_trance_journey]{entranced piece} takes
...
\hyperref[fig:scn_o_29_trance_journey_failed_2]{failed trance-journey example} all
...
make only a single step; e.g. Wave activated by Knight moves like a Pegasus.
Another example, Wave activated by Shaman on capturing-fields moves over that
Shaman's capturing-fields, regardless it can't capture anything.
...
second step based on a choice of the first one. Wave can choose any two steps Unicorn
could make from its starting field and first step-field.
...
Wave activated by Pawn moves toward opponent's figure row, either in a straight line (if
...
Wave activated by such a Pawn also moves horizontally, to the end of a chessboard.
