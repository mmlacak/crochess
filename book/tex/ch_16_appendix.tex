...
\algfmt{CAN}         & classic \algfmt{AN}, as described by FIE  \\
...
notation, depending on what is available.

Losing ability to castle is denoted with \alg{\&\&} (double ampersand).
\hyperref[sec:Appendix/Notation/Castling]{Using previous examples}, if Rooks moved to their
destination fields without castling, it would be written as \alg{R\&\&e1}, and \alg{R\&\&q1},
and in \algfmt{XAN} it would be \alg{R\&\&a1-e1}, and \alg{R\&\&y1-q1}. If Rook in the first
example was captured by opponent's Bishop on neighboring-field, it would be written as
\alg{Ba1*R\&\&}, or as \alg{Bb2-a1*R\&\&} in \algfmt{XAN}.
...
Only last piece participating in a cascade can check or checkmate opponent's King,
because all pieces prior transfer any remaining momentum to next piece in a cascade.

It's possible that on the very next move,
\hyperref[fig:scn_mv_34_activated_piece_checking]{after cascade is finished}, and
after all moved pieces have restored access to all of their step- and capture-fields,
to have opponent's King checked, checkmated. If King is checked on its own player's
turn, checked status is written in front of regular move notation, like so:
...
\textbf{Symbol}                 & \textbf{Side-effect}           & \multicolumn{3}{c}{ \textbf{Accompanying losing tags} }              \\ \cmidrule{3-5}
                                &                                & \emph{castling}      & \emph{promotion}      & \emph{rushing}        \\
\midrule
\alg{*}                         & capturing                      & +                    & +                     & +                     \\ \cmidrule{1-5}
...
targeted piece to lose its promotion, rushing, or castling tag. Losing tags accompanying
...
and row \emph{\alg{*} capturing}.

Some combinations are not possible, which is noted by - (minus sign). For example, Pawn
holding promotion tag cannot be captured by en passant move, as indicated by - at
\emph{promotion} column and \emph{\alg{:} en passant} row. This is so because Pawn can get
promotion tag only on opponent's side of chessboard, while it can be subjected to en passant
...
and \emph{\alg{\&} castling} row. Another instance is a Pawn holding promotion tag which
it can use, found in \emph{promotion} column and \emph{\alg{=} promotion} row.
...
\midrule
\alg{+}              & check                          \\
\alg{\#}             & checkmate, resign              \\ \cmidrule{1-2}
