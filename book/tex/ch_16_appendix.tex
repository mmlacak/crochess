...
\algfmt{/ \textbackslash}   & grouping item(s), enveloped in scope       \\
...
or not, \algfmt{( )} to repeat items 0 or more times, \algfmt{\{ \}} to repeat
items at least once, and \algfmt{/ \textbackslash} to envelop items into a scope.
...
Choices can be limited in scope by enclosing them in \algfmt{/ \textbackslash},
for instance
\begin{alltt}
<abc> = \alg{d} | \alg{e} / \alg{f} | \alg{g} \textbackslash \alg{h} | \alg{i}
\end{alltt}
has two choices. Choice inside group \algfmt{/ \textbackslash} has 2 possible values,
...
values: \alg{d}, \alg{e}\algfmt{/}\alg{f}\algfmt{|}\alg{g}\algfmt{\textbackslash}\alg{h}
...
variant before that will not be using divergence side-effect. Similarly, all
teleportation path separators are not used before Nineteen variant, when first
...
<capturing-active-piece> =
  <classic-piece> | \alg{E} | \alg{U} | \alg{C} | \alg{O} | \alg{G} | \alg{S} | \alg{H}
...
<promote-to-figure> =
  <passive-figure>  | \alg{N} | \alg{B} | \alg{R} | \alg{Q}
| \alg{E} | \alg{U} | \alg{C} | \alg{O} | \alg{G} | \alg{S} | \alg{H} | \alg{I}
