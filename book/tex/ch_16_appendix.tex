...
If Rook in the first example was captured by opponent's Bishop on neighboring-field, it
would be written as \alg{Ba1*R\&\&}, or as \alg{Bb2-a1*R\&\&} in \algfmt{XAN}.
...
\begin{tabular}{ rlccc }
\toprule
\textbf{Symbol}             & \textbf{Side-effect}  & \multicolumn{3}{c}{ \textbf{Accompanying losing tags} }              \\ \cmidrule{3-5}
...
Checkmate and resign notation are both mandatory, to note end of a game score (list of moves
by both players).
...
<pawn-losing-tag> = <pawn-losing-promotion-tag>
                  | \alg{P::}
...
\begin{alltt}
<pawn-sacrifice> =
  \alg{S}<stepping>\alg{~A}<stepping>[\alg{*}[<pawn-losing-tag>]]\alg{;;}
