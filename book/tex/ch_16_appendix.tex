...
also denote another. For instance, \alg{C..j9..p15} might also denote \alg{C.b4.f5..p15}, which happens to
cross \alg{j9} as well.
...
same \hyperref[fig:scn_cot_19_light_light_shaman_interaction_end]{trance-journey example with interactions}
would be written as \\
...
% \alg{[H-f12]\~{}[Wf12-d9]\~{}[Wd9-e13]@[He13..e18*N..m12*P..g24]}.
...
one of rank, file or rank + file, in that order. Disambiguation can be preceded by piece which was demoted.
...
is no plies gathering, since only Monolith moved, there is only one ply.
...
ressurected. If Starchild has been ressurected on a empty field, position is appended after piece.
If there was no suitable empty field at which Starchild could be ressurected, ressurection is not performed,
which is written by appending \alg{\$\$} (double dollar) after the move.

In case \hyperref[fig:scn_o_22_syzygy_starchild_init]{ressurection example} ended by
\hyperref[fig:scn_o_23_syzygy_starchild_end]{ressurecting Queen}, this would be written as \alg{It9\$Q},
and in \algfmt{XAN} it would be \alg{Ii23-t9\$Q}.

If previous example ended by \hyperref[fig:scn_o_24_syzygy_starchild_ressurection]{ressurecting Starchild},
it would be written as \alg{It9\$Iu8}. In \algfmt{XAN}, it would be \alg{Ii23-t9\$Iu8}.

If all neighboring-fields around ressurecting Starchild, all portal-fields around Stars, Monoliths in syzygy,
and all syzygy-fields are occupied, ressurection is not performed, and it is written as \alg{It9\$\$}, in
\algfmt{XAN} it would be \alg{Ii23-t9\$\$}.
...
This \hyperref[fig:scn_n_04_teleport_move_3]{Wave teleporting example}, followed by
