...
just \alg{Cp14} is not good enough since at least 2 different paths lead to the same destination field.
...
Care must be taken to write step which really differentiate paths, otherwise e.g. \alg{C..j9..p15} might
also denote \alg{C.b4.f5..p15}, which happens to cross \alg{j9} as well.
...
followed by, optionally a reached piece, and mandatory by field of displacement. The same
...
Ressurection is written by appending \alg{\$} after the move, followed by piece which was ressurected.
If Starchild has been ressurected on a empty field, position is appended after piece. If there was no
suitable empty field at which Starchild could be ressurected, ressurection is not performed, which is
written by appending \alg{\$\$} after the move.
...
\addcontentsline{toc}{subsection}{Formatting, comments}
\label{sec:Appendix/Formatting, comments}
...
\textbf{Id} & \textbf{Symbol}      & \textbf{Status}                \\
\midrule
1           & \alg{...}            & placeholder for move           \\
2           & \alg{//}             & line comment                   \\
3           & \alg{/*}, \alg{*/}   & comment block                  \\
\bottomrule
\end{tabular}
\caption{Formatting, comments}
\label{tbl:Appendix/Formatting, comments}
\end{table}
