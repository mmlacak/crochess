...
\algfmt{FIDE~C.9.3}.
If \hyperref[fig:scn_mv_26_wave_activation_by_capture_pawn]{Wave activated by Pawn example} had
...
\alg{\%} symbol, so it would be \alg{Bd5\~{}Ah5\%R}.

In \algfmt{XAN}, it would be \alg{Bh9-d5\~{}Ad5-h5\%}. With both plies gathered and converted
piece noted it would be \alg{[Bh9-d5]\~{}[Ad5-h5\%R]}.
...
In this \hyperref[fig:scn_d_17_syzygy_2_stars_init]{syzygy example}, if Monolith was
...
as well. In this \hyperref[fig:scn_tr_15_serpent_activated_wave_ply]{Wave activated by Serpent}
...
if in this
\hyperref[fig:scn_tr_16_wave_out_of_board]{Wave off-board example} dark Knight were located at
...
destination in this \hyperref[fig:scn_mv_33_wave_off_board]{Wave off-board example} is field 2,
...
In this \hyperref[fig:scn_mv_22_activating_rush_pawn_init]{activating Pawns example}, light
...
\hyperref[fig:scn_tr_07_serpent_loop_1]{Serpent activating Pyramid example}, depending on
...
Placeholder for a move is \alg{...} (three dots), usually used to resume game score after
commentary, see \\
...
these are optional in \algfmt{CAN}, see \algfmt{FIDE C.13}. Resigns are written with
\alg{\#\#} (double hashtag) as a complete move, which also ends a game, like so:

\algcycpar
\algcyc{92.}{...}{\#\#}{Dark player resigns.}
\algcycparend

Draw offer is noted with \alg{(=)} (equal sign in brackets), see \algfmt{FIDE C.12}; it's
written immediately following a completed move notation (which might include check), like
so:

\algcycpar
\algcyc{71.}{Nb3+(=)}{...}{Draw offered.}
\algcycparend

Accepted draw offer is noted with \alg{(==)} (two equal signs in brackets) as a complete
move, this ends a game, like so:

\algcycpar
\algcyc{82.}{...}{(==)}{Draw offer accepted.}
\algcycparend

Forced draw, i.e. draw by rules, is written with \alg{(===)} (three equal signs in brackets)
as a complete move, this also ends a game, like so:

\algcycpar
\algcyc{82.}{...}{(===)}{Draw forced, by the rules.}
\algcycparend

In \algfmt{NAN} checks and draw offers are optional; checkmates, resigns, accepted and
forced draws are all mandatory, to set game score end.

Only
\hyperref[fig:scn_mv_35_activated_piece_check_cascade]{last piece in a cascade can check or checkmate}
opponent's King in the same move, because
\hyperref[fig:scn_mv_34_activated_piece_check_init]{all pieces prior transfer any remaining momentum to next piece}
in a cascade.

So, it's possible that immediately at the beginning of the very next opponent's move
to have opponent's King checked, checkmated. This is so because after cascade is finished,
but before next move started, all pieces moved in that cascade have restored access to all
of their step- and capture-fields. If King is checked on its own player's turn, checked
status is written in front of regular move notation, like so:

\algcycpar
\algcyc{91.}{...}{+Nb3}{Checked on own turn, i.e. prechecked.}
\algcycparend

If King is checkmated on its own player's turn, game ends, and only checkmated status
(which is mandatory in \algfmt{NAN}) is written, like so:

\algcycpar
\algcyc{92.}{...}{\#}{Checkmated on own turn, i.e. precheckmated.}
\algcycparend

Player checked on own turn can check opponent, like so:

\algcycpar
\algcyc{91.}{...}{+Nb3+}{Prechecked, then checking opponent.}
\algcycparend

Player in precheck can also checkmate opponent, like so:

\algcycpar
\algcyc{91.}{...}{+Nb3\#}{Prechecked, then checkmating opponent.}
\algcycparend
...
and annotations, like so \alg{ef8*\_!}, \alg{Nb3(=)\_=}. Usage of underscore is mandatory
when annotation can be confused for a regular chess \algfmt{AN}.
...
Check notation is optional in \algfmt{NAN}, just like in \algfmt{CAN}, see \algfmt{FIDE C.9};
precheck notation is also optional. Checkmate, precheckmate and resign notations are mandatory,
to note end of a game score (list of moves by both players).

Draw offer notation is optional in \algfmt{NAN}; accepted draw and draw by rules notations
are mandatory, since they also note end of a game score.
...
\midrule
any own                     & Pyramid                               & except Starchild      \\
any own                     & Wave                                  & including Starchild   \\
Wave                        & any own                               & except King           \\
Wave                        & opponent's Wave                       & ---                   \\
...
\multirow{2}{*}{Starchild}  & \multirow{2}{*}{opponent's Starchild} & only on               \\
                            &                                       & neighboring fields    \\ \cmidrule{1-3}
...
Above, in the Activations table, are listed all the rules which piece can activate
other own pieces, and opponent's Wave, Starchild. Activations are made on step-
and capture-fields, but Starchild can activate only own Wave, Starchild on its
step-fields. Starchild can also activate most pieces on its
...
straight line (if
\hyperref[fig:scn_mv_24_wave_activation_by_step_pawn]{activated on a Pawn's step-field}),
or diagonally (if
\hyperref[fig:scn_mv_26_wave_activation_by_capture_pawn]{activated on a Pawn's capture-field});
