...
\hyperref[fig:scn_o_15_starchild_conversion_immunity_init]{conversion immunity} would be
...
\hyperref[fig:scn_o_23_trance_journey_failed]{failed trance-journey example} all
...
\alg{\~{}} (tilde), before \hyperref[fig:scn_o_26_uplifting_step]{uplifted piece} takes
off. This \hyperref[fig:scn_o_29_dark_piece_sense_journey]{sense-journey example} would
...
\hyperref[fig:scn_o_30_sense_journey_failed]{failed sense-journey example} all
...
written as \alg{Eg15\~{}Wa18||}, and in \algfmt{XAN} it would be
\alg{[Ei11-g15]\~{}[Wg15-a18]||}.

This \hyperref[fig:scn_d_18_teleporting_wave_cascade]{cascading teleportation example} would
be written by sequencing teleportations like so \alg{Eh2\~{}Wb4|Wa24|Wr4\~{}Bt6}, if activated
...
So, losing tag is denoted after piece symbol and its staring position (regardless, if it's
disambiguation or a field), but before any steps, or destination field, depending on what
is available. Writing lost tag is completely optional, it's meant to remind readers what
happened and when.
...
and \alg{R\&\&q1}, and in \algfmt{XAN} it would be \alg{Ra1\&\&-e1}, and \alg{Ry1\&\&-q1}.
Using just disambiguation, it could be writen as e.g. \alg{Ra\&\&..e1}, and \alg{Ry\&\&..q1}. \newline
\indent
...
sign), like so \alg{==e12}. In \algfmt{XAN} the same move would be noted as \alg{Pe11==-e12}.

If the same example contained e.g. dark Bishop at \alg{f12}, Pawn tagged for promotion could
capture it, which would be written as \alg{==f12*B}, and in \algfmt{XAN} as \alg{Pe11==-f12*B}.
When just disambiguation is needed, it could writen as \alg{e==f12*B}, or e.g. as \alg{Pe==.f12*B}.
...
\begin{tabular}{ lll }
\toprule % =======================================================================
\multicolumn{2}{c}{ \textbf{Activated by} }     & \textbf{Moves like}           \\
\cmidrule{1-2} % ................................
...
% \algfmt{/ )}   & grouping item(s), enveloped in scope      \\
...
<active-piece> = <capturing-active-piece>
               | \alg{I}

<capturing-piece> = <capturing-active-piece>
                  | \alg{A}

<disposable-figure> =
<promote-to-figure> = <classic-figure>
                    | <new-active-figure>
                    | <passive-figure>
                    | \alg{I}
...
<stepping-transparency-side-effect> =
  <stepping-no-side-effects>
| [<disambiguation>]
...
% <pawn-losing-promotion-tag> = \alg{P==} | <pawn>
...
<pawn-losing-promotion-tag> = [\alg{P}][\alg{==}]
<trooper-losing-tag> = <trooper>[\alg{::}]
<rook-losing-tag> = \alg{R}[\alg{\&\&}]
<king-losing-tag> = \alg{K}[\alg{\&\&}]

<pawn-losing-tag> = [\alg{P}][\alg{::}]
...
<shaman-terminating-ply> =
  \alg{H}[<disambiguation>]\{<capturing-step>\}

<shaman-capturing-ply> =
  <shaman-terminating-ply><step>
...
%   \alg{I}<stepping>\{\alg{~I}<stepping>\} % sense-journey
...
| <disposable-piece-losing-tag><stepping>
| <king-losing-tag><stepping>
| <shaman-capturing-ply>
| <starchild-resurrecting-ply>
| <pawn-promoting-ply>
...
% | <starchild-cascading-plies> % sense-journey
...
<terminal-ply> =
  <capturing-piece><stepping>
    [<capturing-side-effect>]
| [\{\alg{A}<stepping>\alg{~}\}]<pyramid-promoting-ply>
...
<cascade> =
  <terminating-ply>
| <cascade-start>[\alg{~}<terminal-ply>]

<cascading-start> = \{<cascading-plies>\}

<cascading> =
  \{<cascading-plies>\}[\alg{~}<terminal-ply>]
| [\{<cascading-plies>\}\alg{~}]<terminal-ply>
...
<wave-cascade> =
  \alg{~}<cascade>
| <wave-cascading-start>[\alg{~}<terminal-ply>]
...
\begin{alltt}
<move-king-castling> =
  \alg{K}<stepping-no-side-effects>
  [<castling-side-effect>]
...
| <move-king-castling><status>
| <move-pawn-promotion><status>
| <compatibility-capture>
| <compatibility-castling>
| \alg{#}
| \alg{##}
