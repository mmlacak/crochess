...
In this \hyperref[fig:scn_mv_31_activating_rush_pawn_init]{activating Pawns example},
...
It's possible to have two side-effects accompanying the same step, those are noted
in the order of appearance; so, e.g. capture is writen before promotion. \newline
\indent
As an example, light Pawn on a second to last rank could capture dark Rook, and then
promote after the same step. In One variant, this could be writen as \alg{e26*R=Q}
or, in \algfmt{XAN} it would be \alg{Pd25-e26*R=Q}.
...
piece. Notation for immediate promotion is mandatory, but symbol is not. This gives
us possibility to write previous move as \alg{e26*Q}, which would be interpreted
very differently.
