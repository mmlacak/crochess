...
Note, notational grammar isn't exact, some things are difficult to formalize,
some are too cumbersome. For instance, any ply in trance-journey, teleportation
and elsewhere can be gathered in \alg{[ ]} (square brackets), just like ordinary
ply. Another example, compatibility notation for capture is valid only for
Classical Chess, and nowhere else.
...
% <pawn-sacrifice-capture> =
%     \alg{S}[<disambiguation>]\{.[.]<field>
%       [\alg{*}<pawn>]\}[\alg{|}<field>|\alg{~}<move-new>]
%   | \alg{[S}[<disambiguation>]\{.[.]<field>
%       [\alg{*}<pawn>]\}\alg{]}[\alg{|}<field>|\alg{~}<move-new>]
