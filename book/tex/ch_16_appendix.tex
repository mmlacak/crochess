...
A piece passing "through" another is noted by using \alg{`} (backtick) instead of \alg{.}
(one dot) for a single step; or \alg{``} (double backtick) instead of \alg{..} (two dots)
for multiple steps.

For instance, in \hyperref[fig:scn_mv_07_wave_is_transparent]{this example} light Queen
could capture dark Pegasus, which would be written as \alg{Q``m4``j7..g10*}, or in
\algfmt{XAN} as \alg{Qo2``m4``j7..g10*G}. Should light Wave be positioned immediately
next to light Queen, single backtick would be used instead, like so \alg{Qo2`n3``j7..g10*},
and in \algfmt{XAN} it would be \alg{Qo2`n3``j7..g10*G}.
...
This \hyperref[fig:scn_tr_16_pawn_sacrifice_init]{Pawn-sacrifice} followed by
\hyperref[fig:scn_tr_17_pawn_sacrifice_end]{capturing opponent's Pawns} is written as \\
...
as well. In this \hyperref[fig:scn_tr_21_serpent_activated_wave_ply]{Wave activated by Serpent}
...
if in this
\hyperref[fig:scn_tr_22_wave_out_of_board]{Wave off-board example} dark Knight were located at
...
\alg{.}              & single steps                               \\
\alg{..}             & multiple steps                             \\
\alg{`}              & single step, transparency                  \\
\alg{``}             & multiple steps, transparency               \\
...
First 5 symbols (\alg{.}, \alg{..}, \alg{`}, \alg{``}, and \alg{-}) separates steps
within a single ply. Other symbols (\alg{\~{}}, \alg{|}, \alg{@}, and \alg{;;}) are
separators between plies; or ply and, by extension, move terminators (\alg{||},
\alg{@@}, and \alg{@@@}).
...
\hyperref[fig:scn_tr_18_serpent_activating_wave]{Wave activated by Serpent} is an
...
<movement-step> = \alg{.}[\alg{.}]<field>

<transparency-step> = \alg{`}[\alg{`}]<field>
...
<pawn-sacrifice-captures> =
  \alg{S}[<disambiguation>]
    \{<step>
     | <movement-step>[\alg{*}[<pawn-losing-tag>]]\}
    [<pawn-sacrifice-end>]
