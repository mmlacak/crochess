...
Optional value is used when notation is optional; for instance, most of the time
Pawn is not written, but it's completely fine if it is.

Optional entity represents part of notation that may be present sometimes, but
not always; e.g. demoting to Pawn side-effect is mandatory to write, often
Monolith's movement does not initiate that kind of syzygy.

Note, that optional entity might also contain optional value; e.g. pieces can
loose their tags only sometimes, losing tag notation is also completely optional.
