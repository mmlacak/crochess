...
For official reference FIDE handbook is used: \\
...
\algfmt{CAN} classical algebraic notation, i.e. short form \\
\algfmt{LAN} long algebraic notation, also classical form \\
\algfmt{XAN} extended algebraic notation, new short form
...
movement of the piece would be written as \alg{Gf7}. The same movement in \algfmt{LAN}, would be
...
written as \alg{h5}; writing it as \alg{Ph5} is equally acceptable. Long notation would be
...
Here,
\hyperref[fig:scn_ct_04_pegasus_movement]{Pegasus could capture opponent's Pawn}, which would
be written as \alg{Gg4*P}, or just \alg{Gg4*}, if captured piece is not needed.

In \algfmt{CAN}, the same move would be written as \algcty{Gxg4}. Note, FIDE handbook requires
captures made by Pawn to contain departuring file, \algcty{x}, and then destination field.
...
light Pawn if rushed, which is written as \alg{h3:}.

If disambiguation is needed, it is written as previously described above. Usually, it's enough to add
starting file before destination field. If previous example had additional dark Pawn be located at
\alg{g4}, en passant would be written as \alg{ih3:}.

In \algfmt{CAN}, both en passant and its disambiguation form are written as \algcty{ixh3 e.p}, where
\algcty{e.p.} stands for en passant.
...
In \algfmt{LAN}, the same would be written as \\
