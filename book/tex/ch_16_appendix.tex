...
destination field, or just before movement notation (e.g. individual steps). Usually, this is used
when Pawn is captured, converted, activated, etc., not when it was moved. For instance, in this
...
This means, old side-effects (from Classical Chess) are not necessary to write down in a notation, but
those are in minority, and almost all of new side-effects are necessary. To avoid any possible confusion,
in \algfmt{NAN} all side-effects are mandatory, except tag losing  side-effects (i.e. \alg{==}, \alg{::},
and \alg{\&\&}).

It is recommended to also write optional side-effects and their data, for not much more effort reader
is presented with much easier to understand notation. For example, compare minimalistic notation with
slightly more verbose version: \alg{Re2\~{}Wc2\~{}Pb3*}, vs. \alg{Re2\~{}Wc2\~{}P::b3*N},
...
TODO
\normalsize{}
grammar: fix tag losing side-effects
