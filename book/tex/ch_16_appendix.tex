...
refering to Wave's capture-fields it's just a shorthand for activating piece's
capture-fields. Only Wave activated on a capture-field, and also moving over its
capture-fields can activate Pyramid. \newline
\indent
For instance,
\hyperref[fig:scn_mv_24_wave_activated_by_capture_pawn]{Wave activated by Pawn}
on its capture-field has the same initial steps as
\hyperref[fig:scn_mv_22_wave_activated_by_step_pawn]{Wave activated by Pawn}
on its step-field; but, only the former can activate Pyramids, if moving
diagonally, over its capture-fields.

\TODO :: rewrite

\hyperref[sec:Terms/Activator]{Activator} is material (non-Wave) piece from which
Wave inherits its steps.
