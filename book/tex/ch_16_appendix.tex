...
\algfmt{FIDE~C.9.3}.
If \hyperref[fig:scn_mv_29_wave_activation_by_capture_pawn]{Wave activated by Pawn example} had
...
destination in this \hyperref[fig:scn_mv_36_wave_off_board]{Wave off-board example} is field 2,
...
In this \hyperref[fig:scn_mv_25_activating_rush_pawn_init]{activating Pawns example}, light
...
Only
\hyperref[fig:scn_mv_40_activated_piece_check_cascade]{last piece in a cascade can check or checkmate}
opponent's King in the same move, because
\hyperref[fig:scn_mv_39_activated_piece_check_init]{all pieces prior transfer any remaining momentum to next piece}
in a cascade.
...
straight line (if
\hyperref[fig:scn_mv_27_wave_activation_by_step_pawn]{activated on a Pawn's step-field}),
or diagonally (if
\hyperref[fig:scn_mv_29_wave_activation_by_capture_pawn]{activated on a Pawn's capture-field});
