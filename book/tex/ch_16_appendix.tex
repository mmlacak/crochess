...
In case \hyperref[fig:scn_o_26_syzygy_starchild_init]{ressurection example} ended by
\hyperref[fig:scn_o_27_syzygy_starchild_end]{ressurecting Queen}, this would be written as \alg{It9\$Q},
and in \algfmt{XAN} it would be \alg{Ii23-t9\$Q}.

If previous example ended by \hyperref[fig:scn_o_28_syzygy_starchild_ressurection]{ressurecting Starchild},
...
\subsection*{Step and ply side-effects}
\addcontentsline{toc}{subsection}{Step and ply side-effects}
\label{sec:Appendix/Summary/Step and ply side-effects}
...
\bottomrule
\end{tabular}
\caption{Step and ply side-effects}
\label{tbl:Appendix/Summary/Step and ply side-effects}
\end{table}

Capturing, displacement and failed promotion are both step and ply side-effects, others are purely
ply side-effects. Ply side-effects are those which can occur only on the last step of a ply. Capturing
and displacement as step side-effects can occur only during entranced Shaman's trance-journey. Failed
promotion can be followed by other side-effects in the same step, e.g. en passant; this is why it's
written before destination field.
...
| \alg{*}[\alg{P}[\alg{==}]]
| \alg{<}[<disposable-figure>]<field>
| \alg{<}[\alg{P}[\alg{==}]]<field>

<movement> =
  [<disambiguation>][\alg{==}]<field>
| [<disambiguation>]
    \{.[.]<field>[<step-side-effects>]\}
| <field>\alg{-}<field>
...
<pawn-sacrifice> =
  \alg{S}<movement>\alg{~A}<movement>\alg{*}[\alg{P}[\alg{==}]]\alg{::}
\end{alltt}
