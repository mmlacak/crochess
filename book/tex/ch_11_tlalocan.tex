...
neither in other step- nor capture-direction, even if opponent's piece is present
on a capture-field. \newline
\indent
...
\end{wrapfigure}
Shaman can't capture opponent's piece on a step-field, nor activate Pyramid (in the
example above). Most pieces are not transparent to Shaman, and so when encountered
on its step-fields, Shaman is prevented from moving any further.
