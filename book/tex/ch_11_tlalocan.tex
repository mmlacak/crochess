...
\hyperref[fig:scn_mv_51_static_move_is_illegal_init]{cannot end its move on a starting field}.
...
\hyperref[fig:scn_mv_51_static_move_is_illegal_init]{as with any piece starting a cascade}, it's
...
\hyperref[fig:scn_mv_51_static_move_is_illegal_init]{cannot return to its starting field},
if it's first piece in a cascade (or, the only one in a move). Diverging piece which was
\hyperref[fig:scn_mv_53_static_piece_is_legal_init]{not the first in a cascade can return}
...
\hyperref[fig:scn_mv_47_activating_piece_check_init]{when move is finished}. \newline
\indent
