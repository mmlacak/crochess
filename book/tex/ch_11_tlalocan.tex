...
\noindent
\begin{wrapfigure}[9]{l}{0.35\textwidth} % [11]
\centering
...
dark Shaman), while the one immediately preceding it is called entrancing
Shaman (here, the light one).

Note, trance-journey can be undertaken, but is not obligatory, light player
could just move dark Shaman as
...
4 and 5, in that order. All other (step-)fields are not affected.
...
Interaction with other pieces found on step-fields depends on a color of an
entrancing Shaman, i.e. the one which precedes entranced Shaman (the one which
takes on trance-journey) in a cascade.

If entrancing Shaman is light, all pieces found on affected step-fields can be
moved (but don't have to) to an empty displacement-field chosen by player
commencing trance-journey.

If entrancing Shaman is dark, all pieces, own or opponent's, found on affected
step-fields has to be captured.

Pieces on step-fields not reached by entranced Shaman are not affected.
In all cases, Kings and Stars on a step-field are ignored, they and their fields
are exempt from capturing.
...
\centering
\includegraphics[width=0.208333333\textwidth, keepaspectratio=true]{examples/18_cot/scn_cot_15_displacement_fields.png}
\caption{Displacement-fields}
\label{fig:scn_cot_15_displacement_fields}
\end{wrapfigure}
Displacement-fields are all fields immediately surrounding a particular field
