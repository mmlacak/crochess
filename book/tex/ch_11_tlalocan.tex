...
Shaman can diverge from own Wave, regardless if it's over ordinary (Shamans A, B),
or capture-steps (C, D), over single (B, D), or multiple steps (A, C).
\hyperref[fig:scn_mv_60_diverging_activated_piece_init]{Like before}, activated
Shaman has to have momenum to be able to diverge; otherwise it could only activate
Wave with no momentum.
...
\hyperref[fig:scn_mv_57_own_wave_is_divergent_1]{limited by momenum it had when own Wave was encountered}. \newline
\indent
