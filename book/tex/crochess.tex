...
activation Pyramid would be limited to move at most 4 fields across.
...
in case of Bishop above 4 fields. Again, that is limit of how far Pyramid
could move after activating it.
...
\end{wrapfigure}
\indent
If Pawn hasn't moved yet, it can move up to 4 fields forward. As expected, all passed-by
opponent's Pawns also gain en passant opportunity.

In example on left, if light Pawn's
initial move was 4 fields long, ending in field marked 3, all dark Pawns would have en
passant opportunity. Naturaly, if initial move was only 2 fields long, ending in field
marked 1, only first dark Pawn on the right would gain en passant opportunity.
...
\end{wrapfigure}
\indent
On fields with the same color as Unicorn, it can move the same way
Knight does.
...
\end{wrapfigure}
\indent
If Pawn hasn't moved yet, it can move up to 5 fields forward. As expected, all passed-by
opponent's Pawns also gain en passant opportunity.

In example on left, if light Pawn's
initial move was 5 fields long, ending in field marked 4, all dark Pawns would have en
passant opportunity. Naturaly, if initial move was only 2 fields long, ending in field
marked 1, only first dark Pawn on the right would gain en passant opportunity.
