...
with own other piece, for instance Bishop, and then Pyramid can move further.

Once activated Pyramid moves similar to Rook, only real difference is that
it can move for only so many fields as piece activating Pyramid has moved.
...
Promotion is done by activating Pyramid which then captures field at which
Pawn stands, both Pyramid and Pawn now leaves board as if they were captured,
and desired piece, for instance Queen, is placed on Pawn's field.
...
This can be seen in next example.
...
the board. Conversion is done by activating Pyramid which then captures
field at which opponent's piece stands, now both Pyramid and opponent's piece
leave the board as if they were captured, and the same type of piece as
captured one, now in own color is placed on the field.
...
to be on own side of the board.
