...
\noindent
\begin{figure}[!h]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/10_mv/scn_mv_45_activated_piece_check_init.png}
...
\noindent
\begin{figure}[!h]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/10_mv/scn_mv_46_activated_piece_check_cascade.png}
\vspace*{-1.3\baselineskip}
...
en passant destination field.
Opponent's Pawn then can capture a piece, but cannot capture rushing Pawn anymore. \newline
\indent
On the left, dark Pawn is positioned such that it would gain en passant opportunity,
should light Pawn rush over field P. Rushing Pawn is about to start a cascade by
activating Wave, which will activate Bishop, which will activate light Pyramid.
...
\vspace*{-1.4\baselineskip}
\caption{En passant blocked}
\label{fig:scn_mv_55_en_passant_turned_capture}
\end{wrapfigure}
On the left, now that activated Pyramid has settled onto field P, rushing light
Pawn cascade has finished, and dark Pawn can capture that Pyramid. \newline
\indent
Even though it rushed over field P on the very previous turn, light Pawn cannot
be captured en passant by dark Pawn, since dark Pawn is capturing light Pyramid.

\vspace*{2.7\baselineskip}
\noindent
\begin{wrapfigure}[1]{l}{0.4\textwidth}
\centering
\includegraphics[width=0.39375\textwidth, keepaspectratio=true]{examples/10_mv/scn_mv_56_figure_captured.png}
\vspace*{-1.4\baselineskip}
\caption{Figure captured}
\label{fig:scn_mv_56_figure_captured}
\end{wrapfigure}
. . .
...
\noindent
\TODO \newline
0. Wave is transparent --\textgreater Pawn can rush to full extent, even if its
   path contains multiple Waves ... \newline
1. en-passant blocked, by cascaded opponent's piece in previous move \newline
2. capture preceeding en-passant (by Pawn capturing en-passant, in the same ply),
   due to cascaded own piece in previous move \newline
3. fix grammar for case (2.) \newline
4. Wave is transparent --\textgreater cascaded rushing Pawn can continue cascade,
