...
\subsubsection*{Wave is blocking castling}
\addcontentsline{toc}{subsubsection}{Wave is blocking castling}
\label{sec:Miranda's veil/Wave/Cascading Waves/Wave is blocking castling}

Note, even though Wave is transparent, all step-fields of both King and Rook
must be empty, before they can castle.

\noindent
\begin{figure}[!h]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/10_mv/scn_mv_09_wave_block_castling_king.png}
\caption{Castling King blocked}
\label{fig:scn_mv_09_wave_block_castling_king}
\end{figure}

Above, Wave is blocking light King from castling fully to the left.

\noindent
\begin{figure}[!h]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/10_mv/scn_mv_10_wave_block_castling_rook.png}
\caption{Castling Rook blocked}
\label{fig:scn_mv_10_wave_block_castling_rook}
\end{figure}

Even if King tries to castle short, now Rook A is blocked by the same Wave;
and so this Queen-side castling is illegal.
...
\hyperref[sec:Miranda's veil/Wave/Cascading Waves/Wave is blocking castling]{must be empty},
