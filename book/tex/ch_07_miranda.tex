...
Once started moving, piece has to follow its direction for the remainder of a ply, and
cannot act upon opponent's pieces positioned on its step- or capture-fields outside of
chosen heading. Last piece in a cascade would be able to check opponent's King, only if
that King is positioned on a capture-field in a chosen heading prior to activation of a
piece, i.e. opponent's King would already be checked before cascading move. So, pieces
in a cascade do not attack opponent's King immediately in the very same move in which
they have moved.

Here, light Queen cascades light Waves and Rooks, numbers in lower left corner are
received momentum.
...
1st step determines direction \\
$\rightarrow$ last piece in cascade can only continue to go in determined direction \\
$\rightarrow$ King can't ever be in a check, checkmate until cascade finishes \\
$\rightarrow$ only at the very beginning of the very next move, King can be checked, checkmated \\
