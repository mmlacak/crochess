...
Note, capturing Pawn can take en passant opportunity only if it started a move,
...
\end{wrapfigure}
Again, Pawn can capture another en passant only if it started a move, and cannot
if it's (re)activated. So, capturing Pawn has to be the only piece moving in a
turn, or it has to capture en passant in the very first ply of a cascade.

Here, light Pawn has rushed to its destination, and now is tagged as en passant
opportunity (green marker), grey arrows show path traveled over by the Pawn. Now
it's dark player turn, dark Pawn is about to rush forward and activate dark Wave
A, which would then activate dark Queen, which would activate dark Wave C.
...
tag (and, green marker), and is about to activate dark Wave B, which would
...
As shown here, if activated Pawn could capture en passant, it would be possible
for a Pawn to capture from its initial position; that certainly would not be
"in passing".
