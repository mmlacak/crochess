...
After activation Wave moves like piece that just activated it, with
additional provision that Wave can move over multiple step-fields even
if activating piece can move for only one. Pawn can activate Wave not
just at its' capture-fields, but also on its' step-fields. Wave is not
blocked by any piece, it can continue its' movement freely, as if there
are no pieces on board at all.
...
\label{fig:mw_activation_by_pawns}
% \centering
\end{figure}
