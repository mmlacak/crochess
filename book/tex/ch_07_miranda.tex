...
reactivation, dark Queen still won't attack dark King, even though it's located on
...
\noindent
\begin{figure}[h]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/10_mv/scn_mv_39_cascade_self_checkmate_init.png}
\vspace*{-1.4\baselineskip}
\caption{Cascading opponent's Rook}
\label{fig:scn_mv_39_cascade_self_checkmate_init}
\end{figure}
...
is reactivated and
\hyperref[fig:scn_mv_34_activated_piece_check_init]{moved away from its temporary position}.
Opponent's piece left in a position from where it does check own King after cascade
...
\noindent
\begin{figure}[h]
\includegraphics[width=1.0\textwidth, keepaspectratio=true]{examples/10_mv/scn_mv_40_cascade_self_checkmate_end.png}
\caption{Cascaded self-checkmate}
\label{fig:scn_mv_40_cascade_self_checkmate_end}
\end{figure}

Here, light player's cascade (notice, first piece in a cascade is light Bishop) has
finished. Grey arrows show path travelled over by a piece they point to. Now is dark
player's turn. Light player has left dark Rook in a position to check light King.
Since light King can't be moved out of a check, this is immediate self-checkmate.
