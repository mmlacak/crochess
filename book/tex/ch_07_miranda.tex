...
be captured en passant, since dark Pawn is capturing light Pyramid instead.
...
Pawn can pass over it, and continue rushing to its full extent. Any piece captured
by a Pawn granted en passant opportunity prevents capturing rushed Pawn.

In example on the left similar to previous one, grey arrows show path traveled over
by rushing light Pawn, from its starting field P. Now on dark player's turn, dark
Pawn cannot realize en passant opportunity, and can only capture light Wave instead,
since Wave is positioned on its capture-field E.

The same capturing prevents en passant also applies to rushing Pawns which has been
activated, only difference is that momentum gathered has to be enough to also drive
rushing Pawn.
