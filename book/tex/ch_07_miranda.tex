...
Opponent's King is checked (or checkmated) only after cascade has finished, just like
after normal, non-cascaded move. During cascade, piece can be reactivated on a temporary
field, and thus repositioned to a new field. So, a piece can temporary be located on a
field where it would check (or checkmate) opponent's King, if that would be its final
destination for that cascade. When piece is repositioned from its temporary location,
opponent's King is not affected, and game continues.
...
D. Wave D is now "in-the-air", about ot reactivate light Queen with remaining 5
