...
Available directions % from which one can be chosen
\hyperref[fig:scn_aoa_01_unicorn_same_color]{depend on colors of both Unicorn and its field};
if both are in the same color, Unicorn can do short jump; if colors are different, Unicorn
can do long jump. So, long jump after divergence would be followed by short one, and vice versa,
short jump after divergence would be followed by long one.
...
\vspace*{-0.4\baselineskip}
Here, light Unicorn (now "in the air") after short jump from its starting, same-color
field U, diverged from Wave A located on a dark field, and is about to choose one of long jumps available.
After divergence Unicorn doesn't have momentum, so it cannot activate own Pyramid,
only Wave; it can also capture one of opponent's pieces.
