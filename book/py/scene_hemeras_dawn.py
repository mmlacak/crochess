...
class SceneHemerasDawnMixin:
    ...
    def scn_hd_60_en_passant_grenadier_activated_in_close_quarters(self, bt=BoardType.HemerasDawn):

        scene = Scene( 'scn_hd_60_en_passant_grenadier_activated_in_close_quarters', bt, width=8, height=10.3 )
        ...
        # r --> w(1)
        gen_r_w1 = GS.gen_steps( start=start_r, rels=[ (0, -1), ], include_prev=True, count=8 )
        for i, arr in enumerate( gen_r_w1() ):
            mark_type = MarkType.Action if i == 7 else \
                        MarkType.Legal
            scene.append_arrow( *arr, mark_type=mark_type )

        # w(1) --> w(2)
        gen_w1_w2 = GS.gen_steps( start=start_w_1, rels=[ (-1, 0), ], include_prev=True, count=2 )
        for i, arr in enumerate( gen_w1_w2() ):
            mark_type = MarkType.Action if i == 1 else \
                        MarkType.Legal
            scene.append_arrow( *arr, mark_type=mark_type )

        # w(2) --> g
        gen_w2_g = GS.gen_steps( start=start_w_2, rels=[ (0, 1), ], include_prev=True, count=3 )
        for i, arr in enumerate( gen_w2_g() ):
    ...
    def scn_hd_62_multiple_rushes_init( self, bt=BoardType.HemerasDawn ):

        scene = Scene( 'scn_hd_62_multiple_rushes_init', bt, width=7.3, height=10.3 )
    ...
    def scn_hd_63_multiple_rushes_end( self, bt=BoardType.HemerasDawn ):

        scene = Scene( 'scn_hd_63_multiple_rushes_end', bt, width=7.3, height=10.3 )
        ...
        # --> P
        coords_P_ = GS.gen_steps( end=start_P, rels=[ (0, 1), ], include_prev=True, count=3 )
        for i, arrow in enumerate( coords_P_() ):
            scene.append_arrow( *arrow, mark_type=MarkType.Blocked )

        # --> O
        coords_O_ = GS.gen_steps( end=start_O, rels=[ (0, 1), ], include_prev=True, count=3 )
        for i, arrow in enumerate( coords_O_() ):
            scene.append_arrow( *arrow, mark_type=MarkType.Blocked )
