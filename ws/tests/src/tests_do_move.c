...
bool test_do_move_tag_and_promotion( int index, TestPrints tp )
{
    ...
    result = test_print_failure( cc_rules_do_moves( &game__o, &move_1__t, CC_DME_DoAllMoves ),
                                 TME_Error, "move(s) not done", __FILE__, __LINE__, __func__ )
             && result;
...
bool test_do_move_conversion( int index, TestPrints tp, bool is_failed )
{
    ...
    if ( is_failed )
        cc_chessboard_set_piece( game__o->chessboard, 11, 5, CC_PE_DarkStarchild );
    else
        cc_chessboard_set_piece( game__o->chessboard, 11, 5, CC_PE_DarkShaman );

    cc_chessboard_set_piece( game__o->chessboard, 15, 5, CC_PE_LightPyramid );
    cc_chessboard_set_piece( game__o->chessboard, 21, 11, CC_PE_LightBishop );
    ...
    if ( is_failed )
        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 11, 5 ) == CC_PE_DarkStarchild ),
                                    TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
                 && result;
    else
        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 11, 5 ) == CC_PE_DarkShaman ),
                                    TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
                 && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 15, 5 ) == CC_PE_LightPyramid ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 21, 11 ) == CC_PE_LightBishop ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;
    ...
    if ( is_failed )
        ...
        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 11, 5 ) == CC_PE_LightShaman ),
                                    TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
                 && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 15, 5 ) == CC_PE_LightBishop ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;
...
bool test_do_move_demotion( int index, TestPrints tp )
{
    ...
    cc_chessboard_set_piece( game__o->chessboard, 11, 11, CC_PE_LightBishop );
    cc_chessboard_set_piece( game__o->chessboard, 23, 15, CC_PE_Monolith );
    ...
    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 11, 11 ) == CC_PE_LightBishop ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 23, 15 ) == CC_PE_Monolith ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;
    ...
    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 11, 11 ) == CC_PE_LightPawn ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 22, 22 ) == CC_PE_Monolith ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;
                                ...
                                bool is_oblationing )
{
    ...
    cc_chessboard_set_piece( game__o->chessboard, 23, 15, CC_PE_LightStarchild );
    ...
    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 23, 15 ) == CC_PE_LightStarchild ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;
    ...
    if ( is_failed )
    {
        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 15, 10 ) == CC_PE_LightStarchild ),
                                    TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
                 && result;

        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 16, 11 ) == CC_PE_None ),
                                    TME_Error, "piece found", __FILE__, __LINE__, __func__ )
                 && result;
    }
    else
    {
        if ( is_oblationing )
        {
            result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 15, 10 ) == CC_PE_LightBishop ),
                                        TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
                     && result;
        ...
        else
        {
            ...
            result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 16, 11 ) == CC_PE_LightWave ),
                                        TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
                     && result;
        }
    }
...
bool test_do_move_teleportation_wave( int index, TestPrints tp, bool is_oblationing )
{
    ...
    cc_chessboard_set_piece( game__o->chessboard, 19, 9, CC_PE_Monolith );

    cc_chessboard_set_piece( game__o->chessboard, 10, 12, CC_PE_LightBishop );
    cc_chessboard_set_piece( game__o->chessboard, 8, 14, CC_PE_LightWave );
    cc_chessboard_set_piece( game__o->chessboard, 17, 7, CC_PE_LightKnight );
    ...
    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 5, 11 ) == CC_PE_Monolith ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 19, 9 ) == CC_PE_Monolith ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 10, 12 ) == CC_PE_LightBishop ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 8, 14 ) == CC_PE_LightWave ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 17, 7 ) == CC_PE_LightKnight ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;
    ...
    if ( !ply_2__w ) return cc_game_move_data_free_all( &game__o, NULL, NULL, &plies_0__t, NULL, false );
    ...
    if ( !is_oblationing )
    {
        ...
        if ( !cc_step_none_append( steps_3__t, CC_SLE_Destination, 15, 8, CC_FSUE_User ) )
            return cc_game_move_data_free_all( &game__o, NULL, NULL, &plies_0__t, &steps_3__t, false );
    ...
    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 10, 12 ) == CC_PE_None ),
                                 TME_Error, "piece found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 8, 14 ) == CC_PE_LightBishop ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;
    ...
    {
        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 17, 7 ) == CC_PE_LightWave ),
                                    TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
                 && result;

        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 15, 8 ) == CC_PE_LightKnight ),
                                    TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
                 && result;
    }
...
bool test_do_move_trance_journey( int index, TestPrints tp, bool is_capturing )
{
    ...
    cc_chessboard_set_piece( game__o->chessboard, 6, 9, CC_PE_LightWave );
    cc_chessboard_set_piece( game__o->chessboard, 7, 7, shaman ); // entranced

    cc_chessboard_set_piece( game__o->chessboard, 7, 12, CC_PE_LightBishop ); // 2
    cc_chessboard_set_piece( game__o->chessboard, 5, 1, CC_PE_DarkKnight ); // 4
    cc_chessboard_set_piece_tag( game__o->chessboard, 21, 4, CC_PE_DarkPawn, CC_TE_DelayedPromotion ); // 9
    ...
    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 4, 8 ) == shaman ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 6, 9 ) == CC_PE_LightWave ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 7, 7 ) == shaman ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 7, 12 ) == CC_PE_LightBishop ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 5, 1 ) == CC_PE_DarkKnight ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 21, 4 ) == CC_PE_DarkPawn ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_tag( game__o->chessboard, 21, 4 ) == CC_TE_DelayedPromotion ),
                                 TME_Error, "tag not found", __FILE__, __LINE__, __func__ )
             && result;
    ...
    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 4, 8 ) == CC_PE_None ),
                                 TME_Error, "piece found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 6, 9 ) == shaman ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 7, 7 ) == CC_PE_LightWave ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 21, 4 ) == shaman ),
                                 TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_tag( game__o->chessboard, 21, 4 ) == CC_TE_None ),
                                 TME_Error, "tag found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 7, 12 ) == CC_PE_None ),
                                 TME_Error, "piece found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 5, 1 ) == CC_PE_None ),
                                 TME_Error, "piece found", __FILE__, __LINE__, __func__ )
             && result;

    result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 9, 18 ) == CC_PE_None ),
                                 TME_Error, "piece found", __FILE__, __LINE__, __func__ )
             && result;

    if ( is_capturing )
    {
        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 11, 24 ) == CC_PE_None ),
                                    TME_Error, "piece found", __FILE__, __LINE__, __func__ )
                 && result;
        ...
        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 15, 6 ) == CC_PE_None ),
                                    TME_Error, "piece found", __FILE__, __LINE__, __func__ )
                 && result;

        result = test_print_failure( ( cc_chessboard_get_tag( game__o->chessboard, 15, 6 ) == CC_TE_None ),
                                    TME_Error, "tag found", __FILE__, __LINE__, __func__ )
                 && result;
    }
    else
    {
        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 11, 24 ) == CC_PE_LightBishop ),
                                    TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
                 && result;

        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 1, 5 ) == CC_PE_DarkKnight ),
                                    TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
                 && result;

        result = test_print_failure( ( cc_chessboard_get_piece( game__o->chessboard, 15, 6 ) == CC_PE_DarkPawn ),
                                    TME_Error, "piece not found", __FILE__, __LINE__, __func__ )
                 && result;
