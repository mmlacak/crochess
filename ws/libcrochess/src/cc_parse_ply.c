...
static bool cc_parse_ply( char const * ply_start_an,
    ...
    cc_piece piece_an = cc_piece_from_symbol( piece_symbol, is_light ); // Piece type should be correct, but color (owner) might not be, if it's not first ply.
    ...
    if ( is_first_ply ) {
        ...
        cc_piece pe = cc_chessboard_get_piece( *cb__io, pos.i, pos.j );
    ...
    if ( is_first_ply ) {
        ...
        if ( start && cc_pos_is_valid( start->field ) ) {
            ...
            cc_piece pe = cc_chessboard_get_piece( *cb__io, start_pos.i, start_pos.j );
